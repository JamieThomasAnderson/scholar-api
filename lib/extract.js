...
const waitForJobCompletion = async (jobId) => {
    const response = await axios.get(`https://async.scraperapi.com/jobs/${jobId}`);
...
const linkify = (elementID) => {return elementID ? `${SITE}${elementID}`.split('&')[0] : '';}
...
const extraction = async (URL) => {
    ...
    $('.gs_r.gs_or.gs_scl').each((i, element) => {
        ...
        const publicationText = $(element).find('.gs_a').text().split('-').slice(1)|| [];
        ...
        const citationCount = parseInt(citedByElement.text().replace('Cited by ', '')) || '';

        const relatedArticlesID = ($(element).find('a:contains("Related articles")').attr('href') || '');
        const versionHistoryID = ($(element).find('a:contains("All")').attr('href') || '');

        const authorProfile = authorProfileID ? `${SITE}${authorProfileID}`.split('&')[0] : '';
        const relatedArticles = relatedArticlesID ? `${SITE}${relatedArticlesID}`.split('&')[0] : '';
        const versionHistory = versionHistoryID ? `${SITE}${versionHistoryID}`.split('&')[0] : '';
...
const profileExtraction = async (URL) => {
    ...
    const description = $('meta[name="description"]').attr('content').split('-')
    ...
    $('tr.gsc_a_tr').each((i, element) => {
        ...
        const article = $(element).find('.gsc_a_c a').attr('href').split('&').slice(-1) || '';
        const authors = $(element).find('.gsc_a_t .gs_gray').first().text().split(',').map(author => author.trim()) || '';
        const publication = $(element).find('.gsc_a_t .gs_gray').last().text().split(',')[0] || '';
        const citationCount = parseInt($(element).find('.gsc_a_c a').text()) || '';
...
const academicExtraction = async (URL) => {
    ...
    $('.gsc_1usr').each((i, element) => {
        ...
        const citationCount = parseInt($(element).find('.gs_ai_cby').text().split(' ')[2]) || '';

        const profile = profileID ? `${SITE}/citations?${profileID.split('&')[1]}` : '';
