...
const proxy = async (url, proxyAgent) => {
...
const extraction = async (URL) => {
    const articles = [];
    const proxyAgent = new HttpsProxyAgent.HttpsProxyAgent(`http://${process.env.PROXY_USERNAME}:${process.env.PROXY_PASSWORD}@p.webshare.io:80`);
    const page = await proxy(URL, proxyAgent);
